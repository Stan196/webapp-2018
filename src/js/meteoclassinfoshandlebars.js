...
class Meteo {
  ...
  getMeteoInformations(infos) {
	  ...
	  var maRequete = 'http://api.openweathermap.org/data/2.5/weather?id='+this.infos.villeID+'&units='+this.infos.units+'&lang='+this.infos.lang+'&APPID=0c98af945c8169d1e0fb538cd4ff153f';
    ...
    $.ajax({
      ...
      const context = { "ville": data.name, "temperature": data.main.temp +"ºC", "humidite": data.main.humidity + "%", "tendance": tendance, "imagemeteo": imgsource };
    ...
    return "****** getMeteoInformations() OK. Handlebars.";
